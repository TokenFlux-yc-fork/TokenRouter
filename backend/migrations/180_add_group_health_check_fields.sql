...
COMMENT ON COLUMN groups.health_check_failure_threshold IS '连续失败多少次后触发熔断（标记 unhealthy），默认 3 次';
COMMENT ON COLUMN groups.health_check_success_threshold IS '熔断后连续成功多少次后自动恢复（标记 healthy），默认 2 次';
