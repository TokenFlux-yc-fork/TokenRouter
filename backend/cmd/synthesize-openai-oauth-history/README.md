...
OpenAI OAuth accounts and moves historical usage from OpenAI `upstream`
...
- Source rows are `usage_logs` whose current account has
  `platform='openai'` and `type='upstream'`.
...
The write uses one repeatable-read transaction and an advisory lock. It aborts
unless the number of reassigned rows matches the dry-run plan, no source rows
remain before the cutoff, and the generated accounts own zero rows at or after
the cutoff.
