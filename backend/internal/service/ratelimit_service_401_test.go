...
// OpenAI access-token-only 账号由外部会话维护 token。普通 401 应让当前请求
// failover，但不能据此把账号写成临时或永久不可调度。
...
func TestRateLimitService_HandleUpstreamError_OpenAIOAuth401FilteredSnapshotUsesDatabaseCredentials(t *testing.T) {
	repo := &rateLimitAccountRepoStub{
...
func TestRateLimitService_HandleUpstreamError_OpenAIOAuth401NoMatchingRuleKeepsSchedulable(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	invalidator := &tokenCacheInvalidatorRecorder{}
	service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	service.SetTokenCacheInvalidator(invalidator)
	...
	shouldDisable := service.HandleUpstreamError(
		...
		http.Header{},
		[]byte(`{"error":{"message":"Unauthorized","type":"rejected_by_access_enforcement","code":"no_matching_rule"},"status":401}`),
	)
	...
	require.Equal(t, 0, repo.tempCalls)
	require.Equal(t, 0, repo.updateCredentialsCalls)
	require.Empty(t, invalidator.accounts, "access enforcement 不是 token 过期，不应失效缓存")
}

func TestRateLimitService_HandleUpstreamError_OpenAIOAuth401InvalidatedRemainsPermanent(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	account := &Account{
		ID:       4793,
	...
	shouldDisable := service.HandleUpstreamError(
		...
		http.StatusUnauthorized,
		http.Header{},
		[]byte(`{"error":{"message":"token invalidated","code":"token_invalidated"}}`),
	)
...
func TestRateLimitService_HandleUpstreamError_OpenAIOAuth403HTMLKeepsSchedulable(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	counter := &openAI403CounterCacheStub{counts: []int64{1}}
	service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	service.SetOpenAI403CounterCache(counter)
	...
	shouldDisable := service.HandleUpstreamError(
		...
		http.StatusForbidden,
		http.Header{},
		[]byte(`<html><head><meta name="viewport" content="width=device-width"></head></html>`),
	)
