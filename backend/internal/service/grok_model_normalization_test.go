...
// TestGrokModelNotFoundRemainsRequestScoped 验证 Grok 404 不写入模型级调度状态。
func TestGrokModelNotFoundRemainsRequestScoped(t *testing.T) {
	...
	require.True(t, decision.ShouldFailover(account, http.StatusNotFound, true))
	require.Empty(t, repo.modelRateLimitCalls)
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account))
}

// TestGrokTransientErrorRemainsRequestScoped 验证 Grok 5xx 不累积模型级冷却。
func TestGrokTransientErrorRemainsRequestScoped(t *testing.T) {
	repo := &grokModelStateAccountRepo{}
	svc := &OpenAIGatewayService{rateLimitService: &RateLimitService{accountRepo: repo}}
	account := &Account{
		ID:       4512,
	...
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.False(t, svc.isOpenAIAccountModelRuntimeBlocked(account, "client-alias"))
