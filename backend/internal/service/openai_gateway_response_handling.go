...
func (s *OpenAIGatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string) (*openaiStreamingResult, error) {
	...
	pendingNativePreamble := make([]string, 0, 8)
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if !clientDisconnected {
				if nativeRemoteCompactionV2 && !clientOutputStarted && !startsClientOutput {
					pendingNativePreamble = append(pendingNativePreamble, line)
					return
				}
				if nativeRemoteCompactionV2 && !clientOutputStarted {
					for _, pending := range pendingNativePreamble {
						if !writeBufferedLine(pending) {
							break
						}
					}
					pendingNativePreamble = pendingNativePreamble[:0]
					if clientDisconnected {
						return
					}
				}
