...
func TestAccountTestService_OpenAI401NoRefreshTokenKeepsSchedulable(t *testing.T) {
	...
	require.Error(t, err)
	require.Zero(t, repo.setErrorID, "no-rt OpenAI OAuth account test 401 must not mark account error")
...
func TestAccountTestService_OpenAI401PersonalAccessTokenKeepsSchedulable(t *testing.T) {
	...
	require.Error(t, err)
	require.Zero(t, repo.setErrorID, "PAT account test 401 no_matching_rule must not mark account error")
	require.Empty(t, repo.setErrorMsg)
	require.Equal(t, StatusActive, account.Status)
	require.True(t, account.Schedulable)
}
...
func TestAccountTestService_OpenAI401NoRefreshTokenInvalidatedStillSetsError(t *testing.T) {
	...
	resp := newJSONResponse(http.StatusUnauthorized, `{
		"error":{"message":"Your authentication token has been invalidated.","type":"invalid_request_error","code":"token_invalidated","param":null},
		"status":401
	}`)
