...
func TestOpenAIGatewayService_ForwardCountTokensAsAnthropic_NoRefresh401KeepsSchedulableFromFilteredSnapshot(t *testing.T) {
	...
	account := &Account{
		...
		Type:        AccountTypeOAuth,
		Concurrency: 1,
		// 调度/运行时快照可能过滤掉 token；GetAccessToken 从缓存命中，401 状态处理需回读 DB 识别 no-rt。
		Credentials: map[string]any{},
		Status:      StatusActive,
		Schedulable: true,
	}
	...
	require.Zero(t, repo.tempUnschedCalls, "no-rt OpenAI OAuth 401 must not temp-unschedule from count_tokens")
	require.Zero(t, repo.setErrorCalls, "no-rt OpenAI OAuth 401 must not mark account error from count_tokens")
}
