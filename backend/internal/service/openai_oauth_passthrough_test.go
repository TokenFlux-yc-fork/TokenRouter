...
func TestOpenAIGatewayService_OpenAIPassthrough_RetryableErrorsTriggerFailover(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "apikey_pool_400_capacity_top_level_embedded_message",
			accountType: AccountTypeAPIKey,
			statusCode:  http.StatusBadRequest,
			body:        `{"code":"upstream_error","message":"{\"error\":{\"message\":\"Please retry later.\",\"code\":\"server_is_overloaded\"}} trailing"}`,
