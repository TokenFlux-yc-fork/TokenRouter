...
// 返回非 nil error 表示已接管响应，但下游 SSE 写入或刷新失败。
//
...
func writeOpenAICompactSSEBridge(c *gin.Context, statusCode int, finalResponse []byte) (bool, error) {
	...
	if _, err := c.Writer.Write(payload); err != nil {
		...
		return true, fmt.Errorf("write compact SSE response: %w", err)
	}
	if err := flushOpenAIResponseWriter(c.Writer); err != nil {
		MarkOpsStreamError(c, "downstream_flush_error", err.Error(), 0)
		return true, fmt.Errorf("flush compact SSE response: %w", err)
	}
	return true, nil
}
...
func writeOpenAICompactSSEFailureMessage(c *gin.Context, statusCode int, errType, message string) error {
	...
	if _, err := c.Writer.Write(frame); err != nil {
		MarkOpsStreamError(c, "downstream_write_error", err.Error(), 0)
		return fmt.Errorf("write compact SSE failure: %w", err)
	}
	if err := flushOpenAIResponseWriter(c.Writer); err != nil {
		MarkOpsStreamError(c, "downstream_flush_error", err.Error(), 0)
		return fmt.Errorf("flush compact SSE failure: %w", err)
	}
	return nil
}
