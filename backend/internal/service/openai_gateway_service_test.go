...
func TestOpenAIStreamingPreambleKeepaliveBeforeCapacityDoesNotCommitSemanticResponse(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
...
func TestShouldFailoverOpenAIPassthroughResponse_CapacityAcrossServerErrors(t *testing.T) {
	...
	for _, statusCode := range []int{
		...
		t.Run(strconv.Itoa(statusCode), func(t *testing.T) {
			require.True(t, shouldFailoverOpenAIPassthroughResponse(nil, statusCode, body))
		})
	}
