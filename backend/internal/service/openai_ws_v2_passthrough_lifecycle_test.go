...
func TestPassthroughLifecycle_ResponseCreatedTimeoutClosesWithoutFailover(t *testing.T) {
	...
	case err := <-serverErr:
		...
		require.Contains(t, string(failoverErr.ResponseBody), "first_output_timeout")
	case <-time.After(2500 * time.Millisecond):
		t.Fatal("response.created timeout was left unbounded")
	}
	payload, err := readPassthroughLifecycleFrame(t, clientConn, time.Second)
	require.Error(t, err)
	require.Empty(t, payload, "buffered response.created must not be exposed before failover")
}
