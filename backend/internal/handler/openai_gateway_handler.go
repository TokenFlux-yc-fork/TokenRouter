...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	// body-signal compact：上游 unary 等待期间向下游发 SSE ping/注释行心跳，防止
	...
	for {
		...
		if err != nil {
			...
			if result != nil && result.ImageCount > 0 && !isFailoverErr {
				...
				h.gatewayService.ReportOpenAIAccountScheduleFailure(account.ID, account.GetMappedModel(channelMapping.MappedModel), err)
		...
		if result != nil {
			...
			// ClientDisconnect 以 result,nil 返回，因此仍按上游终态上报并在下方提交 usage。
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	handleWSFailover := func(account *service.Account, failoverErr *service.UpstreamFailoverError) bool {
		...
		if failoverErr.RetryableOnSameAccount {
			...
			if sameAccountRetryCount[account.ID] < retryLimit {
				...
				return sleepWithContext(ctx, sameAccountRetryDelay) && ensureUserSlotHeld()
			}
		}
		releaseAccountSlot()
		if !failoverErr.ShouldRetryNextAccount() {
		...
		if failoverErr.ShouldReportAccountScheduleFailure() {
			tempUnscheduleRetryableFailoverExhausted(ctx, h.gatewayService, account.ID, failoverErr)
		}
		h.gatewayService.RecordOpenAIAccountSwitch()
		failedAccountIDs[account.ID] = struct{}{}
		lastFailoverErr = failoverErr
	...
	for {
		...
		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, wsFirstMessage, hooks); err != nil {
			...
			h.gatewayService.ReportOpenAIAccountScheduleFailure(account.ID, account.GetMappedModel(channelMappingWS.MappedModel), err)
			closeStatus, closeReason := summarizeWSCloseErrorForLog(err)
