	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				...
				WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "enabled", "snapshot", "credentials"}).
					AddRow(tt.identityUnchanged, tt.databaseEnabled, tt.databaseSnapshot, []byte(`{"api_key":"sk-test"}`)))
...
func TestUpdateWithUpstreamBillingProbeEnabledRollsBackWhenOutboxFails(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
		WithArgs(int64(27), service.PlatformOpenAI, service.AccountTypeAPIKey, `{"api_key":"sk-test"}`, nil).
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "enabled", "snapshot", "credentials"}).
			AddRow(true, []byte(`true`), []byte(`{"status":"ok"}`), []byte(`{"api_key":"sk-test"}`)))
