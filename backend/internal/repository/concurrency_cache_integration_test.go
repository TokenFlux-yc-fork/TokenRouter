...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_PreservesParallelInstanceState() {
	accountID := int64(2901)
	userID := int64(2902)
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountSlotKey,
		redis.Z{Score: float64(now), Member: "blue-1"},
		redis.Z{Score: float64(now), Member: "green-1"},
	).Err())
	require.NoError(s.T(), s.rdb.Expire(s.ctx, accountSlotKey, testSlotTTL).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userSlotKey,
		redis.Z{Score: float64(now), Member: "blue-2"},
		redis.Z{Score: float64(now), Member: "green-2"},
	).Err())
	...
	require.NoError(s.T(), s.cache.CleanupStaleProcessSlots(s.ctx, "green-"))

	accountMembers, err := s.rdb.ZRange(s.ctx, accountSlotKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.ElementsMatch(s.T(), []string{"blue-1", "green-1"}, accountMembers)
	userMembers, err := s.rdb.ZRange(s.ctx, userSlotKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.ElementsMatch(s.T(), []string{"blue-2", "green-2"}, userMembers)
	...
	accountIndexScore, err := s.rdb.ZScore(s.ctx, accountActiveIndexKey, strconv.FormatInt(accountID, 10)).Result()
	require.NoError(s.T(), err)
	require.Greater(s.T(), int64(accountIndexScore), now)
	userIndexScore, err := s.rdb.ZScore(s.ctx, userActiveIndexKey, strconv.FormatInt(userID, 10)).Result()
	require.NoError(s.T(), err)
	require.Greater(s.T(), int64(userIndexScore), now)
}
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_PreservesUnindexedWaitCounters() {
	...
	accountWait, err := s.rdb.Get(s.ctx, unindexedAccountWaitKey).Int()
	require.NoError(s.T(), err)
	require.Equal(s.T(), 5, accountWait)
	userWait, err := s.rdb.Get(s.ctx, unindexedUserWaitKey).Int()
	require.NoError(s.T(), err)
	require.Equal(s.T(), 3, userWait)
}

func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_ReapsTTLExpiredIndexedState() {
	...
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	expiredAt := now - int64(testSlotTTL.Seconds()) - 10
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountKey,
		redis.Z{Score: float64(expiredAt), Member: "expired-account"},
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userKey,
		redis.Z{Score: float64(expiredAt), Member: "expired-user"},
	).Err())
