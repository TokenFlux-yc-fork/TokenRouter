...
func (r *scheduledTestPlanRepository) Create(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		INSERT INTO scheduled_test_plans (
			...
			account_circuit_breaker_enabled, failure_threshold, success_threshold,
			failure_cooldown_minutes, timeout_seconds,
	...
	`, plan.AccountID, plan.ModelID, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover,
		plan.AccountCircuitBreakerEnabled, plan.FailureThreshold, plan.SuccessThreshold,
		plan.FailureCooldownMinutes, plan.TimeoutSeconds, plan.NextRunAt)
	return scanPlan(row)
}
...
func (r *scheduledTestPlanRepository) Update(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		SET model_id = $2,
			...
			max_results = $5,
			auto_recover = $6,
			account_circuit_breaker_enabled = $7,
			failure_threshold = $8,
			success_threshold = $9,
...
func scanPlan(row scannable) (*service.ScheduledTestPlan, error) {
	...
	if err := row.Scan(
		...
		&p.AccountCircuitBreakerEnabled, &p.FailureThreshold, &p.SuccessThreshold,
		&p.FailureCooldownMinutes, &p.TimeoutSeconds,
