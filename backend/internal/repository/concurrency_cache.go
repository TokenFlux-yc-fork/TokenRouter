...
// CleanupStaleProcessSlots keeps the historical interface name while reconciling
// only TTL-expired state. A process prefix does not prove that a slot is stale:
// another prefix may belong to a live instance during a rolling deployment.
