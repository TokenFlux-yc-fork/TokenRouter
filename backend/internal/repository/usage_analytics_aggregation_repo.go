...
func (r *dashboardAggregationRepository) GetUsageAnalyticsAggregationState(ctx context.Context) (*service.UsageAnalyticsAggregationState, error) {
	...
	state.CoverageStart = usageAnalyticsNullableTimePointer(coverageStart)
	state.BackfillCursor = usageAnalyticsNullableTimePointer(backfillCursor)
	state.SourceOldestAt = usageAnalyticsNullableTimePointer(sourceOldest)
	state.ManualBackfillStart = usageAnalyticsNullableTimePointer(manualStart)
	state.ManualBackfillCursor = usageAnalyticsNullableTimePointer(manualCursor)
	state.LastRunAt = usageAnalyticsNullableTimePointer(lastRun)
	state.LastSuccessAt = usageAnalyticsNullableTimePointer(lastSuccess)
	state.LastErrorAt = usageAnalyticsNullableTimePointer(lastErrorAt)
...
func (r *dashboardAggregationRepository) GetOldestUsageLogTime(ctx context.Context) (*time.Time, error) {
	...
	return usageAnalyticsNullableTimePointer(oldest), nil
}
...
func usageAnalyticsNullableTimePointer(value sql.NullTime) *time.Time {
	if !value.Valid {
		return nil
	}
