...
TokenRouter can run as a native three-service stack with Apple's `container` CLI. This workflow runs the published TokenRouter, PostgreSQL, and Redis OCI images without Docker Desktop or a Docker-compatible daemon.
...
# Creates volumes/network/containers, waits for dependencies, and starts TokenRouter.
...
# Restart PostgreSQL, Redis, and TokenRouter in dependency order.
...
|---|---|
| Application and gateway variables | Passed to TokenRouter from `.env` |
...
| `REDIS_PASSWORD` | Applied to Redis and TokenRouter |
...
The PostgreSQL volume is mounted at `/var/lib/postgresql`, retaining PostgreSQL 18's default child data directory. TokenRouter and Redis also store data in child directories below their Apple volume mount points. This is required because Apple named volumes do not have Docker's copy-up and mount-point ownership behavior.

## Networking

Apple `container` 1.1 does not provide Compose-style network-scoped service aliases. After PostgreSQL and Redis start, the script reads their current private-network IPv4 addresses from `container inspect`, injects those addresses into a newly created application container, and then starts TokenRouter. The script does not modify `~/.config/container/config.toml` or the macOS host resolver.
...
TOKENROUTER_IMAGE=ghcr.io/tokenflux/tokenrouter:latest # Match APPLE_CONTAINER_SUB2API_IMAGE in .env.
...
- The script targets native `linux/arm64` images. The normal TokenRouter release publishes an arm64 variant.
