...
| `scheduled-test-circuit-breaker` | Apply passive account circuit breaking only after bounded retries and expose scheduled-test controls. | `ef26ee67e` | scheduled-test handler/repository/service, gateway failover | `local` | Scheduled-test, passive-breaker and failover tests. |
