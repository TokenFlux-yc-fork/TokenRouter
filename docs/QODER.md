...
- QoderCN20 refreshes its OpenAPI token and then completes the CN identity status flow. Legacy COSY refresh mode uses the CN Gateway refresh path. PAT sessions are rebuilt from the original CN PAT.
...
The inference profile uses the CN Gateway, `session_type=qoderclicn`, and the pinned CN client version. The wire request includes a top-level `system` string; `chat_context.text` and `chat_context.extra.originalContent` are strings. Inference requests use the COSY session signature and trace context, not the login-time Appcode signature headers.
